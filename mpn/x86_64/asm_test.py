...
x('GLOBL', r'^\s+(\.globl)\s+I\b', lambda m: m[1]) 
x('GSYM_PREFIX', r'^(_?)C:?', lambda m: m[1])
x('RODATA', r'^\s+(\.section)\b.*(\.rodata)', lambda m: m[1] + ' ' + m[2])
x('TYPE', r'^\s+(\.type)\s+I\s*,\s*@\w+', lambda m: m[1] + ' $1,@$2')
x('SIZE', r'^\s+(\.size)\s+I\s*,\s*\d+', lambda m: m[1] + ' $1,$2')
